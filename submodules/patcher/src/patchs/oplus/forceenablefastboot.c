...
bool patch_fastboot_olock1(char* buffer, int32_t size, int32_t global_var_offset) {
    for (int32_t i = 0; i <= size - 4; i += 4) {
        DecodedInst d = decode_at(buffer, i);
        int64_t off0 = calc_adrl_file_offset(buffer, i+4, 0);
        if (!str_at(buffer, size, off0, "if olock secure lock state, Not allow Fastboot.\n")) continue;
        if (off0 < 0) continue;
        if (d.type == INST_CBZ_W || d.type == INST_CBZ_X){//change to B
            write_instr(buffer, i, change_to_b(d.raw));
            printf("Patched fastboot warning jump at 0x%X to always jump, fastboot always enabled\n", i);
...
bool patch_fastboot_olock2(char* buffer, int32_t size, int32_t global_var_offset) {
    for (int32_t i = 0; i <= size - 4; i += 4) {
        DecodedInst d = decode_at(buffer, i);
        int64_t off0 = calc_adrl_file_offset(buffer, i+4, 0);
        if (!str_at(buffer, size, off0, "if olock secure lock state, Not allow Recovery:%d\n")) continue;
        if (off0 < 0) continue;
        if (d.type == INST_BCOND && d.cond == 1){//change to B
            write_instr(buffer, i, change_to_b(d.raw));
            printf("Patched fastboot warning jump at 0x%X to always jump, fastboot always enabled\n", i);
            return true;
        }
...
bool patch_fastboot(char* buffer, int32_t size, int32_t global_var_offset) {
    ...
    if (!patch_fastboot_olock1(buffer, size, global_var_offset)) {
        printf("OPlus Warning: patch_fastboot_olock1 failed\n");
        return false;
    }
    if (!patch_fastboot_olock2(buffer, size, global_var_offset)) {
        printf("OPlus Warning: patch_fastboot_olock2 failed\n");
        return false;
    }
    return true;
}