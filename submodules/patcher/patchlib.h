...
static INT32 track_forward_patch_strb(CHAR8* buffer, INT32 size, INT32 ldrb_off,
    ...
    BOOLEAN cmp_depends_on_tracked_value = FALSE;

    if (anchor_off >= 4) {
        DecodedInst anchor_src = decode_at(buffer, anchor_off - 4);
        if (anchor_src.type == INST_LDR_X_IMM && anchor_src.rn == 31) {
            locset_add_stk64(&set, anchor_src.imm);
            Print_patcher("  Seeded anchor stack source [SP+0x%X]/64 from 0x%X\n",
                   anchor_src.imm, anchor_off - 4);
        }
    }
    ...
    for (INT32 off = ldrb_off + 4; off < size - 4; off += 4) {
        ...
        case INST_STR_X_IMM:
            if (d.rn == 31) {
                if (locset_has_reg(&set, (INT8)d.rt)) {
                    ...
                    locset_print(&set);
                } else if (off > anchor_off && locset_has_stk64(&set, d.imm)) {
        ...
        case INST_LDP_X_IMM:
            if (d.rn == 31) {
                if (locset_has_stk64(&set, d.imm)) {
                    Print_patcher("  0x%X: LDP X%d,X%d,[SP,#0x%X] reload first\n",
                           off, d.rt, d.rt2, d.imm);
                    locset_add_reg(&set, (INT8)d.rt);
                    locset_print(&set);
                } else if (locset_has_stk64(&set, d.imm + 8)) {
                    Print_patcher("  0x%X: LDP X%d,X%d,[SP,#0x%X] reload second\n",
                           off, d.rt, d.rt2, d.imm);
                    locset_add_reg(&set, (INT8)d.rt2);
                    locset_print(&set);
                } else {
                    if (locset_has_reg(&set, (INT8)d.rt)) {
                        Print_patcher("  0x%X: LDP overwrites X%d -> del\n", off, d.rt);
                        locset_del_reg(&set, (INT8)d.rt);
                        locset_print(&set);
                    }
                    if (locset_has_reg(&set, (INT8)d.rt2)) {
                        Print_patcher("  0x%X: LDP overwrites X%d -> del\n", off, d.rt2);
                        locset_del_reg(&set, (INT8)d.rt2);
                        locset_print(&set);
                    }
                }
            }
        ...
        /* ---- CMP Wn, #imm observes the tracked lock value ---- */
        case INST_CMP_W_IMM:
            if (locset_has_reg(&set, (INT8)d.rn)) {
                Print_patcher("  0x%X: CMP W%d,#0x%X depends on tracked value\n",
                       off, d.rn, d.imm);
                cmp_depends_on_tracked_value = TRUE;
            }
            break;

        /* ---- CSET Wd, cond materializes the tracked compare result ---- */
        case INST_CSET_W:
            if (cmp_depends_on_tracked_value && d.rt != 31) {
                Print_patcher("  0x%X: CSET W%d propagates compare result\n", off, d.rt);
                locset_add_reg(&set, (INT8)d.rt);
                locset_print(&set);
                cmp_depends_on_tracked_value = FALSE;
            } else if (locset_has_reg(&set, (INT8)d.rt)) {
                Print_patcher("  0x%X: CSET W%d overwrite -> del\n", off, d.rt);
                locset_del_reg(&set, (INT8)d.rt);
                locset_print(&set);
            }
            break;
...
INT32 find_ldrB_instructio_reverse(CHAR8* buffer, INT32 size,
    ...
    while (now_offset >= 0) {
        ...
        /* ---- ????: LDRB W{current_target}, [Xn!=SP, #imm] ---- */
...
BOOLEAN patch_warning(CHAR8* buffer, INT32 size, INT32 global_var_offset) {
    ...
    //????CBZ
    INT32 max_search_range = warn_off - 64 > 0 ? warn_off - 64 : 0;//?????????????CBZ
