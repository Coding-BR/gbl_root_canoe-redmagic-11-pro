...
typedef struct {
    ...
    UINT8    rt2;       /* second transfer register for pair ops   */
...
static BOOLEAN decode_inst_ldp_x_imm(UINT32 raw, DecodedInst* out) {
    ...
    out->rn   = (raw >> 5) & 0x1F;
    out->rt2  = (raw >> 10) & 0x1F;
    INT32 imm7 = (raw >> 15) & 0x7F;
    if (imm7 & 0x40) imm7 |= ~0x7F;
    out->simm = imm7 << 3;
    out->imm  = (UINT32)out->simm;
    return TRUE;
}
...
static BOOLEAN decode_inst_cset_w(UINT32 raw, DecodedInst* out) {
